...
class SoftwareDeveloperScene:
    ...
    def render_overview(self, screen):
        ...
        sidebar_y = main_y
        button_rect = self._start_button_rect()
        ...
        self.draw_button(screen, button_rect, "Start Workday", font=body_font)
    ...
    def render_optimization(self, screen):
        width, height = screen.get_width(), screen.get_height()
        self.draw_text(screen, "Skill Game 3: Optimization Puzzle", self.title_font, TEXT_COLOR, 60, 40)
        self.draw_text(screen, "Choose the faster algorithm for the login system.", self.body_font, TEXT_COLOR, 60, 95)

        sim_rect = pygame.Rect(60, 140, width - 120, 240)
        draw_panel(screen, sim_rect, (241, 247, 255), border_color=ACCENT_COLOR, border_width=3)
        self.draw_text(screen, "Server Simulation", self.heading_font, TEXT_COLOR, 85, 160)
        ...
        self.draw_text(screen, "Option A: for each user, search entire database", self.body_font, TEXT_COLOR, 80, 410)
        self.draw_text(screen, "Option B: use user ID lookup", self.body_font, TEXT_COLOR, 80, 445)
        self.draw_wrapped_text(screen, self.optimization_feedback, self.body_font, TEXT_COLOR, 60, 490, width - 120)
    ...
    def render_results(self, screen):
        ...
        self.draw_button(screen, self._continue_button_rect(), "Return to Hub")
    ...
    def _start_button_rect(self):
        ...
        return pygame.Rect(
            panel_rect.right - button_width - margin,
            panel_rect.bottom - button_height - margin,
            button_width,
            button_height,
        )
    ...
    def _continue_button_rect(self):
        scale = self._ui_scale()
        button_width = max(150, int(220 * scale))
        button_height = max(42, int(52 * scale))
    ...
    def _hint_button_rect(self):
        ...
        return pygame.Rect(60, height - 70, 140, 50)

    def _action_button_rect(self):
        width, height = self.game.screen.get_size()
        return pygame.Rect(width - 240, height - 70, 180, 50)
    ...
    def _optimization_button_rects(self):
        width = self.game.screen.get_width()
        return pygame.Rect(width - 360, 405, 160, 50), pygame.Rect(width - 180, 405, 160, 50)
