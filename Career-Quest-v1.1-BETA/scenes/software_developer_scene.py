...
class SoftwareDeveloperScene:
    ...
    def _seed_background_shapes(self):
        for _ in range(12):
            self.load_background_shapes.append(
                {
                    "x_ratio": random.uniform(0.18, 0.82),
                    "y_ratio": random.uniform(0.28, 0.82),
                    "size_ratio": random.uniform(0.04, 0.08),
                }
            )
    ...
    def render_optimization(self, screen):
        ...
        (
            ...
            option_b_info_rect,
            feedback_rect,
        ) = self._optimization_layout_rects()

        draw_panel(screen, sim_rect, (241, 247, 255), border_color=ACCENT_COLOR, border_width=3)
        inner_pad_x = max(18, int(24 * scale))
        inner_pad_y = max(16, int(18 * scale))
        self.draw_text(screen, "Server Simulation", heading_font, TEXT_COLOR, sim_rect.x + inner_pad_x, sim_rect.y + inner_pad_y)

        lane_center_y = sim_rect.centery + max(12, int(12 * scale))
        node_width = max(72, int(92 * scale))
        node_height = max(44, int(58 * scale))
        node_gap = max(18, int(24 * scale))
        start_node = pygame.Rect(sim_rect.x + inner_pad_x, lane_center_y - node_height // 2, node_width, node_height)
        end_node = pygame.Rect(sim_rect.right - inner_pad_x - node_width, lane_center_y - node_height // 2, node_width, node_height)
        draw_panel(screen, start_node, (255, 251, 245), border_color=ACCENT_COLOR, border_width=2, radius=12, shadow=False)
        draw_panel(screen, end_node, (255, 251, 245), border_color=ACCENT_COLOR, border_width=2, radius=12, shadow=False)
        self.draw_text(screen, "Users", small_font, TEXT_COLOR, start_node.x + 14, start_node.y + 12)
        self.draw_text(screen, "Server", small_font, TEXT_COLOR, end_node.x + 10, end_node.y + 12)

        track_left = start_node.right + node_gap
        track_right = end_node.x - node_gap
        pygame.draw.line(screen, ACCENT_COLOR, (track_left, lane_center_y), (track_right, lane_center_y), 4)

        good_flow = self.optimization_choice == "B"
        speed = 2.5 if good_flow else 0.8
        packet_radius = max(10, int(13 * min(scale, 1.2)))
        packet_spacing = max(packet_radius * 4, int(72 * scale))
        travel_width = max(packet_radius * 2, track_right - track_left)
        vertical_offset = max(16, int(24 * scale))
        for index in range(8):
            x = track_left + (((self.packet_offset * speed) + index * packet_spacing) % travel_width)
            y = lane_center_y + (-vertical_offset if index % 2 == 0 else vertical_offset)
            pygame.draw.circle(
                screen,
                (78, 167, 120) if good_flow else (193, 102, 82),
                (int(x), y),
                packet_radius,
            )
        ...
        if not good_flow:
            for shape in self.load_background_shapes[:5]:
                x = sim_rect.x + int(sim_rect.w * shape["x_ratio"])
                y = sim_rect.y + int(sim_rect.h * shape["y_ratio"])
                radius = max(6, int(min(sim_rect.w, sim_rect.h) * shape["size_ratio"]))
                pygame.draw.circle(screen, (218, 184, 145), (x, y), radius)
    ...
    def render_results(self, screen):
        ...
        self.draw_text(screen, "Software Developer Report", title_font, TEXT_COLOR, panel_rect.x + 24, panel_rect.y + 20)
        self.draw_wrapped_text(
            screen,
            "You completed a full software release by debugging issues, organizing logic, improving performance, and staying accurate under pressure.",
            body_font,
            TEXT_COLOR,
            panel_rect.x + 24,
        ...
        for index, (label, value) in enumerate(ratings):
            ...
            self.draw_text(screen, stars, body_font, TEXT_COLOR, card_rect.x + 18, card_rect.y + 20 + heading_font.get_height())
    ...
    def _results_button_rect(self):
        ...
        return pygame.Rect(
            ...
            panel_rect.bottom - button_height - max(20, int(28 * scale)),
            button_width,
            button_height,
        )
    ...
    def _results_layout_rects(self):
        ...
        content_top = panel_rect.y + max(110, int(150 * scale))
        gap = max(16, int(20 * scale))
        bottom_space = max(90, int(112 * scale))
        content_height = panel_rect.bottom - bottom_space - content_top

        if panel_rect.w >= 920:
            ratings_width = int(panel_rect.w * 0.48)
            ...
            ratings_height = int(content_height * 0.52)
    ...
    def _results_skill_card_rects(self, ratings_rect):
        ...
        card_height = (ratings_rect.h - gap * 3) // 4
        return [pygame.Rect(ratings_rect.x, ratings_rect.y + index * (card_height + gap), ratings_rect.w, card_height) for index in range(4)]
    ...
    def _optimization_button_rects(self):
        _, option_a_rect, option_b_rect, _, _, _ = self._optimization_layout_rects()
        return option_a_rect, option_b_rect

    def _optimization_layout_rects(self):
        width, height = self.game.screen.get_size()
        scale = self._ui_scale()
        margin_x = max(24, int(width * 0.04))
        top_margin = max(16, int(height * 0.03))
        bottom_margin = max(14, int(height * 0.03))
        gap = max(8, int(height * 0.015))
        title_space = max(36, int(height * 0.08))
        button_gap = max(12, int(18 * scale))
        info_gap = max(8, int(height * 0.015))
        button_height = max(44, int(min(60 * scale, height * 0.10)))
        info_height = max(48, int(height * 0.11))
        feedback_height = max(60, int(height * 0.16))

        available_button_width = width - (margin_x * 2) - button_gap
        button_width = min(max(140, int(210 * scale)), max(120, available_button_width // 2))
        total_button_width = (button_width * 2) + button_gap
        start_x = max(margin_x, (width - total_button_width) // 2)

        feedback_y = height - bottom_margin - feedback_height
        info_y = feedback_y - gap - info_height
        button_y = info_y - info_gap - button_height
        sim_y = top_margin + title_space + gap
        sim_height = max(96, button_y - gap - sim_y)
        sim_rect = pygame.Rect(margin_x, sim_y, width - margin_x * 2, sim_height)

        option_a_rect = pygame.Rect(start_x, button_y, button_width, button_height)
        option_b_rect = pygame.Rect(start_x + button_width + button_gap, button_y, button_width, button_height)
        ...
        feedback_rect = pygame.Rect(margin_x, feedback_y, width - margin_x * 2, feedback_height)
        return sim_rect, option_a_rect, option_b_rect, option_a_info_rect, option_b_info_rect, feedback_rect

    def _optimization_text_rects(self):
        _, _, _, option_a_info_rect, option_b_info_rect, feedback_rect = self._optimization_layout_rects()
        return option_a_info_rect, option_b_info_rect, feedback_rect
