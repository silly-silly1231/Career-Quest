...
class ChefScene:
    ...
    def render_prep_sort(self, screen):
        ...
        station_rect = pygame.Rect(60, 150, width - 120, max(240, height - 310))
        ...
        self.draw_wrapped_text(screen, self.prep_feedback, body_font, TEXT_COLOR, 60, height - 130, width - 120)
    ...
    def render_plating(self, screen):
        ...
        left_rect = pygame.Rect(60, 150, width // 2 - 80, max(250, height - 320))
        right_rect = pygame.Rect(width // 2 + 20, 150, width // 2 - 80, max(250, height - 320))
        ...
        plate_rect = pygame.Rect(right_rect.centerx - 95, right_rect.y + 58, 190, 190)
        pygame.draw.ellipse(screen, (255, 255, 255), plate_rect)
        pygame.draw.ellipse(screen, ACCENT_COLOR, plate_rect, 4)
        for index, rect in enumerate(self._plating_slot_rects()):
        ...
        self.draw_wrapped_text(screen, self.plating_feedback, body_font, TEXT_COLOR, 60, height - 130, width - 120)
        self.draw_button(screen, self._hint_button_rect(), "Hint")
    ...
    def _prep_card_rects(self):
        width, height = self.game.screen.get_size()
        card_width = max(220, (width - 180) // 2)
        card_height = max(90, min(120, (height - 360) // 2))
        return [
            pygame.Rect(90 + (index % 2) * (card_width + 24), 210 + (index // 2) * (card_height + 24), card_width, card_height)
    ...
    def _plating_pool_rects(self):
        width, height = self.game.screen.get_size()
        panel_width = max(250, (width // 2) - 120)
        top = 220
        card_height = max(54, min(74, (height - 380) // 4))
        return [pygame.Rect(90, top + index * (card_height + 16), panel_width, card_height) for index in range(4)]

    def _plating_slot_rects(self):
        width, height = self.game.screen.get_size()
        panel_width = max(250, (width // 2) - 120)
        x = width - panel_width - 85
        top = 360 if height >= 700 else 330
        card_height = max(54, min(72, (height - top - 120) // 4))
        return [pygame.Rect(x, top + index * (card_height + 12), panel_width, card_height) for index in range(4)]
